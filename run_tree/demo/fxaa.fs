...
void main(void)
{
    ...
    // If contrast is lower than a maximum threshold, don't do any AA.
    ...
    // the smaller the final sampling step direction.
    ...
    // Calculate final sampling direction vector by reducing, 
    // clamping to a range and finally adapting to the texture size. 
    samplingDirection = clamp(samplingDirection * minSamplingDirectionFactor, 
                              vec2(-u_maxSpan, -u_maxSpan), vec2(u_maxSpan, u_maxSpan)) * u_texelStep;
